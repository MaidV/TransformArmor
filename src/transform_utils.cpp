...
Article::Article(TESObjectARMO *armor)
    : name(armor->GetFullName()), formID(mask_form<int32_t, 6>(armor)),
      slots(static_cast<int32_t>(armor->GetSlotMask())), enchanted(static_cast<bool>(armor->formEnchanting)),
      form(armor){};
...
void LoadArmors() {
    ...
    bool ignore_enchantments = false;
    ...
    std::vector<BGSKeyword *> badKeywords = {KeywordUtil::GetKeyword("zad_InventoryDevice"),
...
armor_record_t &GetLoadedArmors() { return armor_map; };

void AddItem(Actor *actor, std::vector<TESForm *> &forms, int32_t count = 1, bool silent = false) {
    vector<string> commands;
    for (auto &form : forms)
        commands.push_back("additem " + mask_form<string, 8>(form) + " " + std::to_string(count) + " " +
...
void to_json(json &j, const Article &a) {
    ...
    j = json{
        ...
        {"type", armor_class(a.form)},
        {"keywords", GetKeywordStrings(a.form)},
    };
}
